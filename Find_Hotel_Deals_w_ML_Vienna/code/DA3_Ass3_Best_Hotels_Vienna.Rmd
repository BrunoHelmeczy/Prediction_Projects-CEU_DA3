...
```{r setup, , message = F, warning = F, echo = F, eval = T}
...
```{r models , , warning = F, message = F, echo = F, fig.height= 10, fig.align='center'}
...
**Final Model:** Please see the final models' summary statistics below, re-estimated on the complete dataset. Fitted to the complete dataset, Adjusted R-squared further improved, to 68.3%, with marginal improvements in both MAE & RMSE. Importantly the Root Mean Square Error only represents 16.6% of the average daily room price. With that, the model is ready to find the 5 best deals in town for what presumably is the christmas period.


```{r , , warning = F, message = F, echo = F, fig.height= 10, fig.align='center'}
...
**The 5 Best Deals:** Please see the 5 best deals according to the final model, as well as 5 hotels provided as best deals' according to the reference model for this course. Interestingly, the 5 best deals seem quite diverse in nature & price, incorporating 2 hotels in Innere Stadt of different star rating for 1 night, 1 apartment just over a mile from the centre that is quite budget-friendly, & 2 apartments further from the centre, however 1 of them for 4 nights. Thus, if one sees this analysis' purpose to be finding a diverse variety of alternatives that represent good value-for-money, then the final Random Forest model achieves that purpose, each deal offering a bargain between 44-55 euros according to our model. Meanwhile, the deals offered by the 5 hotels provided as best deals' according to a reference model only represented bargains of 11-18 euros, 1 of the even being over-priced by 28 euros according to the final model. 
...
```{r Hists , warning = F, message = F, echo = F, results= 'asis',  fig.height=2, fig.width=3}
Hists
```

## 2) Scatterplot Associations & Ridge Distributions

```{r Boxes & Scatters, warning = F, message = F, echo = F, results= 'asis',  fig.height=2, fig.width=3}
Boxes_Scatters
```
